...
export default function EmployeeEditPage() {
  ...
  return (
    <div>
      <Link to={`/employees/${id}`} className="text-sm text-indigo-600 hover:underline">← Volver</Link>
      <h2 className="mt-1 text-2xl font-bold text-gray-900">Editar empleado</h2>

      <div className="mt-6 max-w-2xl rounded-xl bg-white p-6 shadow-sm">
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="first_name" className="mb-1 block text-sm font-medium text-gray-700">Nombre</label>
          ...
          <div>
            <label htmlFor="last_name" className="mb-1 block text-sm font-medium text-gray-700">Apellido</label>
          ...
          <div>
            <label htmlFor="department" className="mb-1 block text-sm font-medium text-gray-700">Departamento</label>
            <input id="department" {...register('department')} className={inputBase} />
          </div>

          <div>
            <label htmlFor="position" className="mb-1 block text-sm font-medium text-gray-700">Cargo</label>
          ...
          <div className="flex gap-3 pt-2 sm:col-span-2">
            ...
            <Link to={`/employees/${id}`} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancelar</Link>
          </div>
        </form>
      </div>
    </div>
